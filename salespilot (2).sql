...
CREATE TABLE `subscriptions` (
  ...
  `subscription_status` enum('active','inactive','expired') COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'active',
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `subscriptions_tenant_id_fk` (`tenant_id`),
...
CREATE TABLE `users` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `email` (`email`),
  KEY `users_tenant_id_fk` (`tenant_id`),
...
-- Dump completed on 2025-05-03 10:17:59
