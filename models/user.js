...
class UserModel {
  ...
  static async signup(userData, tenantDomain = 'localhost') {
    try {
      ...
      if (!username || !email || !password || !location) throw new Error('Missing required fields.');
      ...
      await executeQuery(`
        ...
        [userId, subscription_plan, false, tenantDomain]
      );
  ...
  static async requestPasswordReset(email, tenantDomain = 'localhost') {
    try {
      ...
      await executeQuery(`UPDATE users SET reset_token = ?, reset_token_expiry = DATE_ADD(NOW(), INTERVAL 1 HOUR) WHERE email = ?`, [reset_token, email]);
  ...
  static async resetPassword(reset_token, newPassword) {
    try {
      ...
      await executeQuery(`UPDATE users SET password = ?, reset_token = NULL, reset_token_expiry = NULL WHERE id = ?`, [hashedPassword, user[0].id]);
  ...
  static async requestEmailReset(userId, newEmail, tenantDomain = 'localhost') {
    try {
      ...
      console.error('Email Reset Request Error:', error.message);
      throw error;
    }
  }
  ...
  static async confirmEmailReset(emailResetToken, tenantDomain = 'localhost') {
    try {
      ...
      console.error('Email Reset Confirmation Error:', error.message);
      throw error;
    }
  }
  ...
  static async findOne(query, tenantDomain = 'localhost') {
    try {
      ...
      if (!allowedFields.includes(field)) throw new Error(`Invalid query field: ${field}`);
